...
- [2026-08-08]: Personal Entity / GEO / SEO programme — live audit & master plan
  - *Details*: Ran a 14-agent parallel verification across all 10 live web properties (portfolio + `/about/` `/links/` `/blog/` `/blog/publications/`, Mountain Fauna Lover, DeepSafe + dashboard, Evolve, Wealth Compass, Ping-Pong-Counter, Local-File-Diet, SafeSpotter, mattioli.OS, CampFlow) plus 3 off-site surface clusters (academic: ORCID/Scholar/SciProfiles/MDPI/OpenAlex/Semantic Scholar; social: 3 YouTube channels, TikTok, Instagram, LinkedIn, interview video; store/code: Apple Developer, 4 App Store listings, 59 GitHub repos, the `deep-safe` account, and the competing `SimoneMattioli98` account). 276 fetches. Replaced the stale 2026-06-30 audit with an execution-ready master plan.
  - *Key findings*: Person entity fragmented into ~24 nodes where there should be 1; canonical `@id` present on only 2 of 10 properties. **Three surfaces publish false facts**: Google Scholar credits a paper by T Yang & CHC Hsu (supplying all 3 citations and the entire h-index), Semantic Scholar has merged him with the author of a 2015 Bitcoin study, SciProfiles asserts `"Dr."` as the schema `Person.name`. CampFlow canonicalises to a GoDaddy parking page; Mountain Fauna Lover emits `localhost:3000` 76 times. DeepSafe (4.8/1250) and CampFlow (5/12) publish fabricated `aggregateRating` — a site-wide manual-action risk. mattioli.OS and DeepSafe are blank to non-rendering AI crawlers. Homepage `FAQPage` answers contradict the page's own meta description. `llms.txt` has 3 of 7 links returning 404. Social surfaces contain zero positioning tokens. JSON-LD validity itself is clean estate-wide — the previously-reported double-encoding bug is fixed and did not recur.
  - *Tech Notes*: New `SEO_AUDIT/README.md` (master plan: 8 P0 + 17 Phase-1 + 13 Phase-2 + 8 Phase-3 + 10 Phase-4 tasks with stable IDs ENT-/CON-/ITA-/SAT-, verification steps and hard ordering constraints). Full evidence in `SEO_AUDIT/2026-08-08-entity-audit-evidence.md`. Previous audit archived as `SEO_AUDIT/2026-06-30-audit-archived.md`. Manual actions with copy-paste text appended to `TO_SIMO_DO.md`. Canonical Person URI fixed as `https://simo-hue.github.io/#person` — must stay byte-identical and language-neutral across the Italian rollout. No code changes made yet.

- [2026-08-08]: Phase 1 — single source of truth for entity facts, Person node rebuilt
  - *Details*: Identity facts lived in six independent places and three had already drifted: the homepage FAQ said "is a Computer Science student at the University of Verona" while the meta description on the same page said "CS graduate and EIT Digital Master's student", and llms.txt claimed a master's at KTH. Cross-checking `data/timeline.yml` showed why this matters: he graduated Verona in 2025 and has **not started the master's** — ELTE begins September 2026, KTH August 2027 — so every surface claiming he studies at KTH was a year early. Claiming a credential before it is true is the same failure mode as the "Dr." SciProfiles wrongly asserts.
  - *Tech Notes*: New `data/entity.yml` is the canonical source for name, jobTitle, status, description, education, publications, profiles, ventures, apps, projects and FAQ. `about.yml` keeps presentation only. One `profiles:` list drives both the visible icon row and schema `sameAs` via `display`/`same_as` flags, so a profile can no longer be linked in the UI but missing from the graph (the bug that left ORCID on /links/ but out of `sameAs`). Google Scholar and Semantic Scholar carry `same_as: false` + `blocked_by`, because both still publish false facts (a paper by T Yang & CHC Hsu; a 2015 Bitcoin study) — flipping one boolean ships them once ENT-04/ENT-15 are done. `schema-person.html` rebuilt as an `@graph`: Person (canonical `@id`, ORCID `identifier`, 8 `sameAs`, `alumniOf` completed-education only) + one University of Verona node with a Wikidata `sameAs` replacing three anonymous duplicates + both ventures with `founder` → the Person + the BDCC ScholarlyArticle with its DOI and all four authors in order. **`worksFor` removed** — it asserted employment by Verona where the relation was enrolment. `schema-website.html` gained an `@id`. `schema-faq.html` now renders 6 answers from entity.yml. New `schema-entity-ref.html` emits a Person/WebSite/WebPage reference on every non-home page, adding `ProfilePage` on /links/ (matched on `.Type`, since /links/ has an empty `.Section`), `Blog` on /blog/ only, and `CollectionPage` on blog sub-sections so they do not mint competing Blog nodes. `llms.txt` is now a generated output format built from entity.yml; `static/llms.txt` deleted. `robots.txt` rewritten: adds ClaudeBot (previously absent — only the retired `anthropic-ai`/`Claude-Web` were listed) plus OAI-SearchBot, Applebot-Extended, Perplexity-User and others, drops the misplaced `Crawl-delay`, narrows the `/*.json$` block that was hiding the PWA manifest, and declares all 8 sitemaps. TikTok handle corrected in `data/timeline.yml` (the wrong `@mountainfaunalove` was there, not in links.yml as the audit reported). `timeout = "300s"` added to hugo.toml — cold-cache builds were timing out on image-heavy posts. **Verified** with `hugo --gc --minify`: 611 pages, no errors or deprecation warnings; every JSON-LD block on 6 page types parses; the canonical Person `@id` is present on all of them (was 2 of 5); `worksFor` absent; 0 occurrences of the stale "student at the University of Verona"; all 17 site URLs in the generated llms.txt return 200 (3 of 7 previously 404'd).

- [2026-08-08]: Phase 2 — six placeholder posts removed from the index; three replaced with real build stories
  - *Details*: Six posts were live, indexed and publicly advertising themselves as unwritten. Three said "Future article about X" (ELTE, KTH, TalTech) for events that have not happened — ELTE starts September 2026, the Tallinn kick-off is October 2026, KTH is August 2027 — and three said "The complete article will be written here by the author" (Evolve, Ping Pong Counter, Warranties Vault). All six are now `draft: true`; the app posts additionally carry ~1,000-word build stories awaiting Simone's answers to the inline `[SIMONE: ...]` markers.
  - *Tech Notes*: Posts are engineering case studies, not product marketing, deliberately: each app already has a landing page that must keep ranking, and a second page restating its features would compete with it. Drafted by three agents reading the actual repos (`mattioli.OS` for Evolve, `Ping-Pong-Counter`, and a clone of `Mobl` for Warranties Vault), then put through two adversarial fact-check rounds plus a surgical polish. **Round 1 returned NEEDS_FIX on all three with 21 unevidenced claims** — the most serious being a `keycheck` CloudKit record described as a security mechanism that appears nowhere in any source file, plus a fabricated first-person rationale for Evolve's central architectural decision, a quoted UI string that does not exist, and past tense implying a `markError` bug was fixed when the code still contains it. Round 2 resolved all 21 and was independently re-verified against the source ("no laundered evidence found"), surfacing 12 smaller precision issues; round 3 fixed those. Errors went invented-mechanism → overstated-qualifier across the rounds. Front matter corrected too: all three shared the copy-pasted description "A premium Flutter application designed for modern mobile experiences" (wrong for the two Swift apps), and Warranties Vault linked to `simo-hue/Warranties-Vault`, which 404s — the real repo is `Mobl`. App Store and product-page links added. **Verified** with a clean `hugo --gc --minify --cleanDestinationDir`: 583 pages, no errors, all six drafts excluded, and zero occurrences of placeholder text or `[SIMONE:` markers in the output. Note for future local builds: Hugo does not clean `public/` without `--cleanDestinationDir`, so deleted pages linger locally and can make verification read stale.

- [2026-08-08]: Phase 2 mechanical — crawl budget reclaimed, 197 MB removed from the published site
  - *Details*: Two unrelated problems. (1) Tag archives were a third of the sitemap. Measurement settled it: 166 distinct tags across ~60 posts, of which 146 have exactly one post, 14 have two, 6 have three or four, and none has five or more — the largest tag aggregates 4 posts, so no tag page holds enough to rank while all of them consumed crawl budget as near-duplicates of pages already in the sitemap. (2) The published site was 750 MB, roughly 75% of the GitHub Pages 1 GB limit.
  - *Tech Notes*: `seo/noindex.html` now covers all `tag` terms and the `/tags/` index (previously only single-post tags), plus `/sections/` and `/offline/`. Sitemap went 115 → 75 URLs; all 60 posts and 9 `passion` pages retained, `passion` deliberately unaffected as a curated navigation taxonomy. `/offline/` is matched on `RelPermalink` because it renders as `Kind=page` with an EMPTY `.Section`, so a Section test silently never fires. `basic-seo.html` changed from `noindex, nofollow` to **`noindex, follow`**: tag archives exist to link through to posts, and nofollow told crawlers to ignore exactly those links. On assets: `og-image.png` (495 KB) → `og-image.jpg` at q90 (125 KB, same 1200×630) with `params.toml` updated — JPEG rather than WebP because several social platforms still fail to render WebP for `og:image`. Deleted `static/images/favicon.png` (229 KB) and the dead `layouts/index.webmanifest` that was its only referrer; `layouts/_default/index.webmanifest` is the live template, confirmed by the built manifest being byte-identical after removal. Downscaled 40 oversized source images to 2400 px max (several were 6000×4000 camera originals), 251 MB → 160 MB. Removed **124 redundant image originals totalling 105 MB**: every one had a `.webp` twin, every post's front matter `image:` is `.webp`, and no `index.md` referenced the original by name — verified that no template globs bundle resources by type (all `Resources.GetMatch` calls take an explicit name). All are recoverable from git history. **Verified** with a clean rebuild: 583 pages, no errors, 731 JSON-LD blocks all valid, and all **2,344** local image references resolve to files that exist. Published output 750 MB → 553 MB.

- [2026-08-08]: Thin posts — four more placeholders withdrawn, Mountain Fauna Lover rewritten
  - *Details*: Auditing the 16 sub-300-word posts by *prose* rather than total word count split them cleanly. Four were placeholders with essentially no body: `experiences/GDG AI Hackathon` (a bare `## Introduction`, zero words of prose) and three book notes reading **"COMMING SOON, Still Reading"** (sic), dated November 2024 and live for roughly 21 months. Five more are short but genuinely finished pieces of writing (Budapest 105 words of prose, GISEP Rimini 125, Sicily 137, Poland 143, Italy Trip 210). One, `mountainfaunalover`, is a project post whose facts are externally verifiable.
  - *Tech Notes*: The four placeholders are now `draft: true`, each with a comment recording why and what unblocks it — withdrawn rather than padded, since none can be expanded without the author's own material. `mountainfaunalover` expanded 138 → 471 words in source (382 rendered) and kept live: everything added is checkable (Val di Rabbi / Stelvio / Trentino locations, what digiscoping is, the four channels, the website that was missing from its links), with nothing invented about his experience. Its `description` was the placeholder "A brief description of Mountain Fauna Lover presence on Social media Platforms"; tags gained digiscoping/trentino/stelvio and `wild life` was corrected to `wildlife`. **Third instance of the wrong TikTok handle**: this post linked `@mountainfaunalove` (no *r*, a stranger's account), which was rendering on 9+ pages through post cards — after `data/timeline.yml` earlier and the audit's mistaken report that it was in `links.yml`. The four `[SIMONE: ...]` questions drafted for this post were stripped to `TO_SIMO_DO.md` rather than published, because unlike the app-post placeholders this page is live and taking it down to await answers would be a regression. The five short-but-written posts are deliberately left alone: the only honest route to 600 words is his memories, and padding them with generic travel-guide copy about the Danube or the Colosseum would make them worse — undifferentiated content is what retrieval systems discard, and their whole value is being first-hand. **Verified** with a clean build: 583 pages, no errors, all four withdrawn posts absent from the output, and zero occurrences site-wide of placeholder text, the wrong TikTok handle, or `[SIMONE:` markers.
